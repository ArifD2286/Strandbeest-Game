...
@export var radius: float = 12.5
@export var charge_radius: float = 5.0
@export var burst_radius: float = 15.5
...
func _process(delta: float) -> void:
	...
	match player._state:
		"Idle":
			...
			position.x = _rest_position.x + current_radius * cos(_omega * _t)
			position.y = _rest_position.y + current_radius * sin(_omega * _t)
		"Charging":
			_t += delta
			_omega = TAU / cycle_seconds
			...
			position.x = current_centre.x + current_radius * cos(_omega * _t)
			position.y = current_centre.y + current_radius * sin(_omega * _t)
		"Burst":
			_t += delta
			_omega = TAU / cycle_seconds
